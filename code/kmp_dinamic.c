...
double benchmark_brute_force(const char *text, const char *pattern, int *shifts, int text_size, int pattern_size, int *count);
...
void run_benchmark(const char *file_path, const char *pattern, int pattern_size, int multiplier, int k, FILE *results_file) {
    ...
    for (int i = 0; i < k; i++) {
        double execution_time = benchmark_brute_force(text, pattern, shifts, file_size, pattern_size, count);
...
int main() {
    ...
    const char *output_file = "kmp_dinamic_results.txt"; // Output file to store results
